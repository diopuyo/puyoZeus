"""固定journal背景へ同callの色置換veto候補だけを追加する。"""
...
B = load('_palette_journal_cli', ROOT.parent / 'g2_atomic_journal_runtime_2026-09-09_v1/live_cli.py')
F, configuration = B.F, B.configuration


def observer() -> Any:
    return load('_palette_evidence_adapter', ROOT.parent / 'g2_palette_evidence_veto_2026-09-09_v1/adapter.py')
...
def bootstrap() -> tuple[Any, Any, Any, Any]:
    ...
    def install(stack: Any, collector: Any, history: Any, state: Any) -> None:
        ...
        journal.install(stack, collector, history, state, enabled=True)
