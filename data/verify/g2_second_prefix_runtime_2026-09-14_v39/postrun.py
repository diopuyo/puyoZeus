"""A39の終端着弾を原J/原prefix数学/新保存票で検収し、M1のpending照合へ渡す。"""
...
def modules(stack: Any, owner: Any, parts: Any) -> tuple:
    ...
    inactive_packets = lines(output / 'SECOND_INACTIVE_END.jsonl')
    inactive_status = read(output / 'SECOND_INACTIVE_END_STATUS.json')
    consumer = N(**(vars(consumer) | dict(Replay=T.A.INACTIVE_SAVED.derive(
        consumer, inactive_packets, inactive_status))))
