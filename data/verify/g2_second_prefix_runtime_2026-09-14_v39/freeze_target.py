"""既存PASSを保持し、同世代終了の残件だけを検証するA39固定票。"""
...
OLD = T.ROOT.parent/'g2_second_prefix_runtime_2026-09-14_v38'
...
def evidence() -> list[str]:
    ...
    tests = list(prior['test_receipts']) + [str(POST/name) for name in TESTS]
    for path in tests:
    ...
    creator = read(POST/'creator_terminal_v39d/RESULT.json')
