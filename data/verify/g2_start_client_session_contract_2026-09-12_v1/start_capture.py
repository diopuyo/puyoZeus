...
def score_reset_threshold(tracker: Any) -> int:
    """原会計methodの実定数を読む。候補の固定値と違えば黙って許可しない。"""
    method = type(tracker).on_state_transition
    value = method.__globals__['SCORE_RESET_THRESHOLD']
    Q.require(type(value) is int and value == Q.SCORE_RESET_THRESHOLD, 'original_score_reset_threshold')
    return value
...
class Capture(A.Capture):
    def __init__(self, loop: Any, tail: Any, identity: dict[str, str]) -> None:
        ...
        self.score_reset_threshold = score_reset_threshold(self.loop.runtime_state['ojama_tracker'])
    ...
    def snapshot(self) -> dict:
        try:
            ...
            value['start_parameters'] = dict(debounce_sec=self.debounce, visual_persist_sec=self.visual_persist,
                                              score_reset_threshold=self.score_reset_threshold)
