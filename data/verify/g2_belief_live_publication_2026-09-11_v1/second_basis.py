...
def qualify(evidence: Any, witness: Any, pipe: Any) -> tuple[Any, Any]:
    ...
    C.require(sm.context.frame_idx == row['frame'] and sm.context.state.value == 'stable'
        and pipe._active_chain_2p is None, 'second_basis_current')
...
def initialize(evidence: Any, witness: Any, pipe: Any, registry: Any, factory: Any,
               deadline: int, policy: Any) -> tuple[Any, dict[str, Any]]:
    ...
    row, grid = qualify(evidence, witness, pipe)
