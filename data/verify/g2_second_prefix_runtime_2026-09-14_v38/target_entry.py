...
import handoff_gate as H
W = A.W
sys.path.append(str(A.PREVIOUS.SAFETY))

ROOT = Path(__file__).resolve().parent
GO = ROOT / 'TARGET_GO.json'
OUTPUT_NAME = 'video38_second_prefix_candidate_v38'
THREADS = 2
FIXED_END_FRAME = W.LAST  # 原診断区間の終了frame。通知検証も同じ分母で照合する。
...
def runtime_sources() -> tuple[Path, ...]:
    ...
    paths = [p for root in (ROOT, A.PREVIOUS.PRIOR) for p in root.glob('*.py')
...
def observe(main: Any, output: Path) -> int:
    ...
    try:
        ...
        notices = sys.modules['_g2_second_notice_saved_v21'].verify(output, FIXED_END_FRAME)
        common.write(output / 'SECOND_SETTLED_NOTICE_REVIEW.json', notices)
        common.write(output / 'EARLY_INPUT_SAVED_REVIEW.json', H.verify_saved(output, FIXED_END_FRAME))
