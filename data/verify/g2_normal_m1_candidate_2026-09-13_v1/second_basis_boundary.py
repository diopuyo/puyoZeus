"""左右明示の元資格を保持し、actionのみの境界は基準未発行HOLDにする。"""
...
def wrapped(original: Any, basis: Any) -> Any:
    def qualify(evidence: Any, witness: Any, pipe: Any, side: str = '2P') -> Any:
        held = False

        def require(ok: bool, reason: str) -> None:
            nonlocal held
            if not ok and reason == 'second_basis_generation':
                step = witness.pair(evidence.latest['frame'])[0 if side == '1P' else 1]
        ...
        result = selected(evidence, witness, pipe, side=side)
        if held:
