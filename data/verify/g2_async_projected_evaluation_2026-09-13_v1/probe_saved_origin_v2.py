...
def main() -> None:
    ...
    trigger = next(e['active_origin']['trigger_sec'] for e in journal['events'] if e.get('active_origin'))
    result = P.project(value, value.scope, origin, origin_frame=FRAME, cutoff_frame=FRAME,
        origin_token=TOKEN, origin_trigger_sec=trigger)
    targets = ('PROBABILISTIC_BASIS.jsonl', 'atomic_journal.jsonl')
    code_paths = (Path(__file__), Path(P.__file__), Path(S.__file__), Path(P.B.__file__),
        Path(P.calculate_chain_score.__code__.co_filename))
    receipt = dict(code_sha256={str(p): hashlib.sha256(p.read_bytes()).hexdigest() for p in code_paths},
        origin_available_frame=FRAME, origin_trigger_sec=trigger, source_run=str(SOURCE), source_files={name:
    ...
    with (ROOT / 'SAVED_ORIGIN_PROJECTION_v2.json').open('x') as stream:
