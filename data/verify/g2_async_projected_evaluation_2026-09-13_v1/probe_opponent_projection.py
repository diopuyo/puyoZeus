...
ANCHOR, CUTOFF, FPS = 34864, 35672, 60
SIDE, TOKEN, ORIGIN_ID = '2P', 'step:6621', 130710770750704
...
def inputs() -> tuple[dict, dict, dict]:
    ...
    assert len(found) == 1 and found[0]['frame_idx'] == CUTOFF
    step = found[0]
...
def main() -> None:
    ...
    receipt = dict(kind='non_owned_deterministic_projection_diagnosis/v1', side=SIDE,
        observed_frame=ANCHOR, origin_available_frame=CUTOFF, cutoff_frame=CUTOFF,
    ...
    with (ROOT / 'OPPONENT_PROJECTION_v1.json').open('x') as stream:
