...
def construct_and_close(module: Any, arrival: Any, state: dict, output: Path) -> None:
    ...
    with pytest.raises(RuntimeError) as caught:
        with ExitStack() as stack:
            ...
            B.verify(state, require_instance=True, require_projected_input=True)
            capture = value.projected_origin_binding.capture
            result = capture.completed(journal.history.frame)
            assert result['projected_input']['status'] == 'HOLD'
            assert result['projected_input']['reason'] == 'both_side_J_not_available'
            assert capture.projected_input is not None
    ...
    assert capture.projected_input is None
    saved = json.loads((output / 'PROJECTED_ORIGIN_CAPTURE.jsonl').read_text())
    assert saved['projected_input']['status'] == 'HOLD'
    assert receipt['original_error'] == repr(planned)
    assert json.loads((output / 'BELIEF_M1_SESSION.json').read_text())['error'] == repr(planned)


def test_projected_callback_binding_scope_close(monkeypatch: Any, tmp_path: Path) -> None:
    prepare(monkeypatch)
    ...
    with ExitStack() as stack:
        selection_path = B.ROOT.parent / 'g2_prefix_lane_integration_2026-09-13_v1/prefix_selection.py'
        selection = original_loader.load('_projected_factory_prefix_selection', selection_path)
        adapter = selection.dependencies(selection.private_loader(stack, original_loader.load))
        live = N(module=N(B=module.B.B))  # 初回J欠測の入口検査。実Live所有陽性ではない。
        lease = N(closed=False, current=lambda: (live, adapter))
        scope = stack.enter_context(ExitStack())
        state = B.install(stack, bootstrap, replace, prefix_lease=lease, binding_stack=lambda: scope)
        ...
        B.verify(state)
        with pytest.raises(ValueError, match='instance_not_created'): B.verify(state, require_instance=True)
        ...
        scope.close()
        assert state['binding']['closed']
        with pytest.raises(ValueError, match='binding_closed'): B.verify(state)
        with pytest.raises(ValueError, match='binding_closed'):
            bootstrap.load('_g2_pub_runtime_live_session', Path(module.__file__))
        with pytest.raises(ValueError, match='binding_closed'):
            patch.scheduled(bootstrap.load, base_cls)
        bootstrap.load('_actual_install_runtime_patch', B.PATCH)
