"""元Scheduled/最終型監査より前に2P到来Sessionを合成する。旧監査は緩めない。"""
...
BASE = ROOT.parent / 'g2_second_terminal_arrival_2026-09-14_v1'
PREFIX = ROOT.parent / 'g2_second_pending_prefix_2026-09-14_v3/selection.py'
PATCH = ROOT.parent / 'g2_m1_completion_candidate_2026-09-12_v1/second_mode_binding.py'
...
class Selection:
    ...
    def dependencies(self, load: Any) -> tuple:
        if not self.modules:
            ...
            for alias, (name, injection) in zip(NAMES, specs):
                ...
                self.modules[alias] = load(alias, (ROOT if name in ('warning_source.py', 'observed_terminal_drop.py') else BASE) / name, values)
    ...
    def bind(self, module: Any, replace: Any, load: Any) -> None:
        if self.bound:
            require(self.bound[0] is module and module.session_class is self.bound[2], 'factory_replaced')
            return
        before = module.session_class
        require(Path(before.__code__.co_filename).resolve() == PREFIX, 'prefix_factory_order')
        def factory(original: Any, arrival: Any) -> type:
            self.claim()
            base = before(original, arrival)
            require(Path(arrival.__file__).resolve() == ARRIVAL, 'arrival_owner')
        ...
        self.bound[:] = (module, before, factory)
        replace(self.stack, module, 'session_class', factory)
...
def install(stack: Any, bootstrap: Any, replace: Any, scope_getter: Any = None) -> Selection:
    ...
    def load(alias: str, path: Any, injection: Any = None) -> Any:
        ...
        if Path(path).resolve() == PATCH:
            selected.bind(value, replace, original)
        return value
