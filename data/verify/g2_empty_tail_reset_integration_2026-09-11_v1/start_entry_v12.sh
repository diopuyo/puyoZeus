...
test ! -e "$root/prefix_cpu_v12" && test ! -e "$root/entry_v12.log" || exit 92
...
./venv/bin/python "$root/run_entry.py" prefix_cpu_v12 > "$root/entry_v12.log" 2>&1 < /dev/null
result=$?
printf '\nACTUAL_CHILD_EXIT=%s\n' "$result" >> "$root/entry_v12.log"
exit "$result"
