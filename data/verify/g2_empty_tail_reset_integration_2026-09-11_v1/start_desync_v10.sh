...
test ! -e "$root/prefix_cpu_v10" && test ! -e "$root/desync_v10.log" || exit 92
...
./venv/bin/python "$root/run_desync.py" prefix_cpu_v10 > "$root/desync_v10.log" 2>&1 < /dev/null
result=$?
printf '\nACTUAL_CHILD_EXIT=%s\n' "$result" >> "$root/desync_v10.log"
exit "$result"
