...
test ! -e "$root/prefix_cpu_v4" && test ! -e "$root/continuation_v4.log" || exit 92
...
./venv/bin/python "$root/run_continuation.py" prefix_cpu_v4 > "$root/continuation_v4.log" 2>&1 < /dev/null
result=$?
printf '\nACTUAL_CHILD_EXIT=%s\n' "$result" >> "$root/continuation_v4.log"
exit "$result"
