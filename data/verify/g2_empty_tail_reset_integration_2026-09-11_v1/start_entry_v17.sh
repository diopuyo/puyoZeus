...
test ! -e "$root/prefix_cpu_v17" && test ! -e "$root/entry_v17.log" || exit 92
...
./venv/bin/python "$root/run_entry_v6.py" prefix_cpu_v17 > "$root/entry_v17.log" 2>&1 < /dev/null
result=$?
printf '\nACTUAL_CHILD_EXIT=%s\n' "$result" >> "$root/entry_v17.log"
exit "$result"
