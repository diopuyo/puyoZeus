...
test ! -e "$root/prefix_cpu_v18" && test ! -e "$root/entry_v18.log" || exit 92
...
./venv/bin/python "$root/run_entry_v7.py" prefix_cpu_v18 > "$root/entry_v18.log" 2>&1 < /dev/null
result=$?
printf '\nACTUAL_CHILD_EXIT=%s\n' "$result" >> "$root/entry_v18.log"
exit "$result"
