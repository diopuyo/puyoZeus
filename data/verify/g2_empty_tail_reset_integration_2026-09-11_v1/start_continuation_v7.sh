...
test ! -e "$root/prefix_cpu_v7" && test ! -e "$root/continuation_v7.log" || exit 92
...
./venv/bin/python "$root/run_continuation.py" prefix_cpu_v7 > "$root/continuation_v7.log" 2>&1 < /dev/null
result=$?
printf '\nACTUAL_CHILD_EXIT=%s\n' "$result" >> "$root/continuation_v7.log"
exit "$result"
