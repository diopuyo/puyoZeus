...
test ! -e "$root/prefix_cpu_v16" && test ! -e "$root/entry_v16.log" || exit 92
...
./venv/bin/python "$root/run_entry_v5.py" prefix_cpu_v16 > "$root/entry_v16.log" 2>&1 < /dev/null
result=$?
printf '\nACTUAL_CHILD_EXIT=%s\n' "$result" >> "$root/entry_v16.log"
exit "$result"
