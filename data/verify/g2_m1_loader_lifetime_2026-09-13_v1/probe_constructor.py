...
def main() -> None:
    output = ROOT / 'session_constructor_v1'
    ...
    with ExitStack() as outer:
        ...
        try:
            with ExitStack() as inner:
                ...
                assert client.child.poll() is None and session.schedule_ready
                assert session.physical.Mode is parts.mode.BASE.Mode
                basis = type(session).__mro__[2].basis.__globals__['B']
                assert Path(basis.qualify.__code__.co_filename).resolve() == R.CANDIDATE / 'second_basis_boundary.py'
                assert basis.initialize.__globals__['qualify'] is basis.qualify
                raise RuntimeError(STOP)
        except RuntimeError as error:
            assert str(error) == STOP
        session, client = refs['session'], refs['client']
        assert client.closed and client.child.returncode == 0
        assert session.witness.closed and session.evidence.closed and session.evaluation_flags.closed
        assert not session.saved and session.schedule_rows == 0
    assert Path(basis.qualify.__code__.co_filename).name == 'second_basis.py'
    report = dict(original_Session_constructor=True, original_parent_client_ready_and_closed=True,
        child_exit=client.child.returncode, thermal=thermal, second_type_selected=True,
        original_initialize_bound_to_boundary_and_restored=True, flags_and_witness_restored=True,
