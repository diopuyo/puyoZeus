...
def pipeline(c: Any, mode: str) -> Any:
    ...
    event = SimpleNamespace(trigger_sec=CHAIN_FIRST / T.T.C.FPS, mechanism='formula_read', chain_count=4, total_score=TOTAL_SCORE)
    def update(frame_idx: int, time_sec: float, frame: Any) -> Any:
        result = original(frame_idx, time_sec, frame)
        if frame_idx == CHAIN_FIRST:
            result.p1.chain_event = event
...
def main() -> None:
    ...
    saved = run(real, ROOT / 'positive_full_v3', 'positive', end)
    ...
    with (ROOT / 'POSITIVE_PROBE_v3.json').open('x', encoding='utf-8') as stream:
