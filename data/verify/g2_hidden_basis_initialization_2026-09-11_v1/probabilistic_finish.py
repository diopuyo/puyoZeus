...
PAST_OFFSETS = (6, 4, 2)
TARGET_KEYS = frozenset(('basis_only', 'actual_J_steps', 'probabilistic_basis_frame',
    'native_consumptions', 'physical_transitions', 'old_archive_unchanged',
    'integer_continuation_verified', 'full_finalizer_verified', 'quality_gate_clear', 'actual_video_run'))
...
def saved(output: Path, serializer: Any) -> tuple[Any, dict[str, Any]]:
    ...
    require(TARGET_KEYS <= target.keys(), 'target_required_fields')
...
def audit_votes(output: Path, packet: Any, canonical: Any, value: Any) -> None:
    ...
    for vote in evidence['votes']:
        frame = vote['frame']
        require(vote['exit_used_as_vote'] is False and vote['frames'] == [frame - offset for offset in PAST_OFFSETS], 'past_votes')
...
def live(kept: Any) -> dict[str, Any]:
    ...
    anchor = read(state['output'], 'PROBABILISTIC_FACTORY_ANCHOR.json')
    require(anchor['ready'] is True and anchor['error'] is None
            and anchor['factory_id'] == id(factory) and anchor['registry_id'] == id(connection.registry)
            and anchor['binding_id'] == id(connection.binding) and anchor['source_call_token'] == connection.binding.initial_call_token
            and anchor['frame'] == current.frame and anchor['scope'] == list(current.scope),
            'factory_anchor')
