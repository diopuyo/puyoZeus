...
OLD = T.ROOT.parent / 'g2_second_prefix_runtime_2026-09-14_v34'
TERM = T.A.TERMINAL
...
TESTS = tuple(TERM / name for name in (
    ...
    'terminal_notice_after_v3.xml')) + (T.ROOT / 'm1_timeline_v1.xml',)
...
def inherited() -> None:
    for path, digest in read(OLD / 'TARGET_GO.json')['frozen_files'].items():
        T.require(hashlib.sha256(Path(path).read_bytes()).hexdigest() == digest,
                  'A34_source_changed:' + path)
    for name in UNCHANGED:
        expected = (OLD / name).read_text().replace(OLD.name, T.ROOT.name).replace(
            'video38_second_prefix_candidate_v34', T.OUTPUT_NAME)
...
def evidence() -> None:
    ...
    constructor = read(T.ROOT / 'FINAL_CONSTRUCTOR_v4.json')
    T.require(all(constructor[key] for key in ('actual_A35_configuration', 'aliases_released',
        'source_tap_restored', 'sources_unchanged')), 'terminal_constructor_unchecked')
    saved = read(T.ROOT / 'SAVED_MODULES_v1.json')
    T.require(all(saved[key] for key in ('actual_A35_saved_loader', 'shared_prefix_family_type',
...
def main() -> None:
    ...
    value = dict(decision='GO_fixed_whole_target_observation', output=str(T.ROOT.parent / T.OUTPUT_NAME),
        ...
        known_connection_failures=[], parent_decision=parent, independent_reviews=[312, 313, 315],
