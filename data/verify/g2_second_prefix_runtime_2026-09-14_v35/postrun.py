"""A35の終端着弾を原J/原prefix数学/新保存票で検収し、M1のpending照合へ渡す。"""
...
def modules(stack: Any, owner: Any, parts: Any) -> tuple:
    ...
    math = selected('terminal_math', TERM / 'observed_terminal_drop.py')
    warning = selected('warning', TERM / 'warning_source.py')
    consumer = selected('terminal', TERM / 'terminal_saved.py', dict(
