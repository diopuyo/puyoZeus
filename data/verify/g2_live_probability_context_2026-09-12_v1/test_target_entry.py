...
def test_probability_runtime_sources_are_pinned() -> None:
    ...
    assert all(not name.startswith(('test_', 'probe_')) for name in names)
...
def test_raw_finalizer_without_generation_or_go(monkeypatch: pytest.MonkeyPatch,
    ...
    if child_exit == 0:
        ...
        Path(str(output) + '.resources.jsonl').write_text(json.dumps(dict(pid=os.getpid(), safety_stop=False)) + '\n')
