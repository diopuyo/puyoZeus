...
OUTPUT_NAME = 'video38_history_publication_probe_whole_start_v1'
THREADS = 2
...
def runtime_sources() -> tuple[Path, ...]:
    ...
    paths = [p for p in ROOT.glob('*.py') if not p.name.startswith(('test_', 'probe_'))]
    return tuple(sorted(paths + [ROOT / 'run_whole_target.sh']))
...
def main() -> int:
    ...
    parser.add_argument('--mode', choices=('observe', 'finalize'), required=True)
