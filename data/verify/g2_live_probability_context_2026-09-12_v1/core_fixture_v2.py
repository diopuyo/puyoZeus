...
def replace(result: Any, refs: dict) -> Any:
    ...
    snapshot, baseline, occurrence, side_factory = (vars(c)['perform'].__globals__['derived'] for c in classes[:4])
    side = sys.modules[classes[3].__module__]
    assert vars(side) is vars(classes[3])['perform'].__globals__, 'side_namespace_not_live'
    deferred = Deferred()
    core = owned('_g2_core_fixture_context', 'context.py').core(classes[-2], deferred, END_FRAME)
    current = snapshot(baseline(occurrence(side_factory(core))), side)
