...
def replay() -> dict:
    ...
    try:
        ...
        snapshots = {}
        for side, handle in capture.handles.items():
            snapshot = capture.ledger.snapshot(handle)
            snapshots[side] = dict(instance_id=handle.instance_id, generation=asdict(snapshot.generation),
                episode_count=len(snapshot.episodes), prediction_count=len(snapshot.predictions),
                origin_prediction_revision=snapshot.origin_prediction_revision,
                predictions=[asdict(prediction) for prediction in snapshot.predictions])
...
def main() -> None:
    ...
    try:
        ...
        os.chdir(initial)
        sys.path[:] = paths
    with (ROOT / 'REPLAY_CANDIDATE_v2.json').open('x', encoding='utf-8') as stream:
