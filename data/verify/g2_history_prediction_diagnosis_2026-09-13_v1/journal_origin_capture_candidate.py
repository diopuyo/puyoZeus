...
class OriginCapture:
    def __init__(self, witness: Any, journal: Any, pipe: Any, ledger_module: Any,
        ...
        self.reprojection_holds: dict[str, dict[str, str]] = {side: {} for side in R.SIDES}
    ...
    def consume(self, row: dict, *, available_frame: int | None = None) -> dict:
        ...
        if handle is not None and (scope_changed or self.ledger.snapshot(handle).generation != current):
            ...
            self.reprojection_holds[side].clear()
    ...
    def observe(self, side: str, generation: Any, point: dict, raw: dict,
        ...
        if key in self.reprojection_holds[side]: return self.reprojection_holds[side][key]
        ...
        handle = self.handles.get(side)
        if handle is None and event.mechanism != CHAIN_MECHANISM_LANDING:
            return 'first_event_not_landing'
        prediction = self.simulator.simulate(event.before_board)
        if prediction.chain_count == 0:
            return self.hold_zero(side, generation, point, raw, prediction, event.before_board, recorded_frame)
    ...
    def hold_zero(self, side: str, generation: Any, point: dict, raw: dict,
                  prediction: Any, board: Any, recorded_frame: int | None) -> str:
        """正しい零連鎖だけを理由付き非予測にする。不正なresultは診断停止する。"""
        values = (prediction.chain_count, prediction.total_erased,
                  prediction.total_ojama, prediction.participating_cells)
        if (any(type(value) is not int or value != 0 for value in values)
                or prediction.steps or prediction.final_board.to_dict()['grid'] != board.to_dict()['grid']):
            raise ValueError('origin_capture_malformed_zero_prediction')
        reason = 'origin_reprojection_zero_chain_hold'
        self.stream.write(encoded(dict(kind='origin_reprojection_hold', side=side, reason=reason,
            generation=asdict(generation), source_scope=self.scopes[side],
            recorded_frame=point['frame_idx'] if recorded_frame is None else recorded_frame,
            available_frame=point['frame_idx'], raw=raw, prediction=None,
            ledger_modified=False, physical_identity_verified=False, quality_gate_clear=False,
            future_fire_power_supply_authorized=False)) + '\n')
        self.stream.flush()
        self.reprojection_holds[side][encoded(raw)] = reason
        return reason
