...
VERSION = 'observed_warning_lower_bound/v1'
...
def select(check: Any, rows: tuple[dict, ...], scope: tuple, current_frame: int,
    ...
    check(all(row['context_known'] and row['active'] is True
              and row['own_chain_active'] is False and row['own_score_delta'] == 0
    ...
    return dict(kind=VERSION, scope=scope, cutoff=frame, first_positive=first,
