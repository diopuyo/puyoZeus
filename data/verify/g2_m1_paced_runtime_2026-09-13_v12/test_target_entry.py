...
def test_raw_finalizer_without_generation_or_go(monkeypatch: pytest.MonkeyPatch,
    ...
    if child_exit == 0:
        ...
        resource = dict(pid=os.getpid(), safety_stop=False, schema='g2_ram_thermal_guard_v1',
            nvidia_smi_status='ok', nvidia_smi_error=None, hw_thermal='Not Active', sw_thermal='Not Active',
            gpu0_temperature_c=50, stop_reasons=[], cpu_package_temperature_c=None, process_start_ticks='123')
...
def test_new_runtime_and_review_sources() -> None:
    ...
    assert T.THREADS == 1
    assert T.OUTPUT_NAME == 'video38_m1_paced_candidate_v12'
