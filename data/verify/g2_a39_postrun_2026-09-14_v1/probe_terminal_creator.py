...
import sys
import time
sys.path.insert(0, str(Path(__file__).resolve().parent.parent/'g2_a38_postrun_2026-09-14_v1'))
    ...
    try:
        with ExitStack() as stack:
            ...
            with selected.__globals__['S'].configured():
                ...
                records = list((A.INACTIVE if version == 'v40' else T).TRACE)
                ...
                result['reader_bind_records'] = list(A.READER.TRACE)
                assert len(A.READER.TRACE) == 1, 'missing_terminal_reader_actual_bind'
