...
**粒度仮説 (未検証、次セッションへの申し送り)**: 1盤面シミュレーションが
2µs前後と極めて速いため、並列化の単位 (フロンティア1ノード=最大22配置の
展開、約44µs分の仕事) がrayonのタスク分配オーバーヘッドに対して細かすぎる
可能性がある (CPU飽和と関係なく、この粒度では8スレッド化の恩恵が
出にくい構造的な問題かもしれない、という仮説)。確度は低い (計測なしの
仕事量からの逆算であり `cargo flamegraph` 等での実測はしていない)。
再ベンチ時にこの仮説も一緒に検証すること (もし空きCPUがあってもスケール
しないなら、フロンティア単位でなくフロンティア×配置単位でのバッチ化
(numpy版がやっていたような一括simulate) への再設計が必要になる)。

**再現性確認 (2026-08-12実施)**: `target/` を削除した完全クリーン状態から
`maturin develop --release` を再実行し、パリティテスト4件が全て再度
パスすることを確認済み (ビルド手順の再現性は検証済み)。
