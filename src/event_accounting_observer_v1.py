...
LEGACY_ACCOUNTING_OBSERVER_VERSION = "event-accounting-observer/v1"
ACCOUNTING_OBSERVER_VERSION = "event-accounting-observer/v1.1"
SUPPORTED_ACCOUNTING_OBSERVER_VERSIONS = frozenset({
    LEGACY_ACCOUNTING_OBSERVER_VERSION,
...
class EventAccountingRecorder:
    ...
    def _classify(
        ...
        formal_boundary = self._advance_boundary(game_idx, t_sec, deltas)
    ...
    def _advance_boundary(
        self, game_idx: int, t_sec: float, deltas: dict[str, int],
    ) -> bool:
        changed = self._last_game_idx is not None and game_idx != self._last_game_idx
        gross_reset = any(
            deltas[f"boundary_resets_{side}"] > 0 for side in ("p1", "p2")
        )
        if changed or gross_reset:
