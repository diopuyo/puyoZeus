...
from src.event_accounting_observer_v1 import (
    ...
    SUPPORTED_ACCOUNTING_OBSERVER_VERSIONS,
)
...
ACCOUNTING_ADAPTER_VERSION = "event-accounting-adapter/v1.4"
COUNTER_NAMES = tuple(
...
class _EventBuilder:
    ...
    def _build_row(self, row: AccountingRow) -> None:
        self.accounting_source_order = 0
        close_reason = _provisional_boundary_close_reason(row)
        if close_reason is not None:
            for side in SIDE_ORDER:
                self._close_provisionals(row.frame_idx, side, close_reason)
...
def _parse_sidecar(value: Mapping[str, Any]) -> EventAccountingSidecar:
    ...
    observer_version = value.get("observer_version")
    if (not isinstance(observer_version, str)
            or observer_version not in SUPPORTED_ACCOUNTING_OBSERVER_VERSIONS):
...
def _parse_row(
    ...
    formal_boundary = _formal_boundary_value(value)
    if residual != (0, 0):
...
def _formal_boundary_value(value: Mapping[str, Any]) -> bool:
    if "formal_boundary" not in value:
        raise EventAccountingAdapterError("正式境界フラグがありません")
    raw = value["formal_boundary"]
...
def _provisional_boundary_close_reason(row: AccountingRow) -> str | None:
    if row.formal_boundary:
        return "match_boundary"
    if any(row.deltas[f"boundary_resets_{side}"] > 0 for side in SIDE_ORDER):
        return "gross_boundary_reset"
    return None
