...
class BoardCNN(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.block1 = _conv_block(in_channels, CONV1_CHANNELS)
        self.block2 = _conv_block(CONV1_CHANNELS, CONV2_CHANNELS)
        self.block3 = _conv_block(CONV2_CHANNELS, CONV3_CHANNELS)
        self.pool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Linear(CONV3_CHANNELS, embed_dim)
    ...
    def forward(self, board: torch.Tensor) -> torch.Tensor:
        ...
        h = self.block2(h)
        h = self.block3(h)
        h = self.pool(h)              # (B, CONV3, 1, 1)
        h = h.flatten(start_dim=1)    # (B, CONV3)
...
class SiameseBoardCNN(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.cnn = BoardCNN(in_channels=in_channels, embed_dim=embed_dim)
        concat_dim = embed_dim * 3  # [emb_1p ; emb_2p ; emb_1p - emb_2p]
        self.winrate_head = _build_head(concat_dim, head_hidden, 1)
        self.indicator_head = _build_head(
            concat_dim, head_hidden, n_indicators
        )
