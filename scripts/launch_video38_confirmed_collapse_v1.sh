...
setsid -f env PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. CUDA_VISIBLE_DEVICES=0 \
  ...
  "$project_root/venv/bin/python" -u -m scripts.diagnose_video38_confirmed_collapse_v1 \
  --start-sec 560 --end-sec 583 --allow-native-runtime-mismatch --output-root "$output_root" \
  > "$log_path" 2>&1 < /dev/null
sleep 1
diagnostic_pid="$(pgrep -n -f "^${project_root}/venv/bin/python -u -m scripts.diagnose_video38_confirmed_collapse_v1 .*--output-root ${output_root}$")"
