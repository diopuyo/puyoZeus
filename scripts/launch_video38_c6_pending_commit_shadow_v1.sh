...
output_root="${1:?新規C6 pending shadow出力directoryを指定してください}"
...
setsid -f env PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. CUDA_VISIBLE_DEVICES=0 \
  ...
  -m scripts.diagnose_video38_c6_pending_commit_shadow_v1 \
  --start-sec 560 --end-sec 605 --allow-native-runtime-mismatch \
  --output-root "$output_root" > "$log_path" 2>&1 < /dev/null
sleep 1
pending_pid="$(pgrep -n -f "^${project_root}/venv/bin/python -u -m scripts.diagnose_video38_c6_pending_commit_shadow_v1 .*--output-root ${output_root}$")"
printf 'PID=%s\nLOG=%s\nOUTPUT=%s\n' "$pending_pid" "$log_path" "$output_root"
