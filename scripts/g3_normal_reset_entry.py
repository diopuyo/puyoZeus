"""正常M1の実loadへ観測/採録reset保留を限定接続する。"""
...
G = R.G
NORMAL = G.ROOT / 'data/verify/g2_normal_m1_candidate_2026-09-13_v1/second_observation.py'
...
def install(stack: Any, adapter: Any, output: Path) -> None:
    ...
    def create(load: Any, frames: tuple[int, ...]) -> Any:
        ...
        G.require(bootstrap.load is load, 'normal_reset_initial_loader')
        def selected(alias: str, path: Any, injection: Any = None) -> Any:
            module = load(alias, path, injection)
            target = Path(path).resolve()
            if target in (NORMAL, FLAGS):
                G.require(target not in fired, 'normal_reset_duplicate_load')
                receipt = (N.install if target == NORMAL else N.install_flags)(stack, module, replace)
                fired.add(target)
                G.save(output / ('G3_NORMAL_RESET_' + target.stem + '.json'),
...
def main() -> int:
    ...
    for module in (sys.modules[__name__], N):
        G.require(str(Path(module.__file__).resolve()) in plan['entry_pins'], 'normal_reset_entry_pin')
