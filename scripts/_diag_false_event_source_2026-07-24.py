"""真因計装(2026-07-24): 偽連鎖イベントの発生源特定(chain_detector vs 早期発火)。

完全 read-only 診断。src/ は一切変更しない。

背景:
    recognition_physics_review.py の _measure_false_chain_events が
    「早期発火の48%が連鎖ゼロの疑似発火」と計測済みだが、その trigger が
    VideoChainTracker (puyo数減少検知、src/chain_detector.py) 由来か、
    機能B(score急増早期発火)/機能D(掛け算式検知早期発火、
    enable_chain_formula_detection=True が既定)由来かは未分離。
    機能B/Dの疑似 ChainEvent は total_erased=0 固定(simulateを経由しない)、
    VideoChainTracker由来の本物イベントは total_erased>=4(4連結消去の定義上)。
    この差を使い、偽イベントの発生源を機械的に分離する。

Usage:
    PYTHONPATH=. ./venv/bin/python scripts/_diag_false_event_source_2026-07-24.py
"""
...
OUTPUT_DIR: Path = PROJ_ROOT / "data" / "verify" / "diag_false_event_source_2026-07-24"

# recognition_physics_review.py と同じ対象窓 (再現性のため揃える)。
