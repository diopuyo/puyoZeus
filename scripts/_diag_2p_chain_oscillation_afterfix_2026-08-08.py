"""2P 連鎖中 chain⇄他状態 振動の原因調査: バグB+C 修正後の効果測定
(読み取り専用・使い捨て計装、2026-08-08)。

_diag_2p_chain_oscillation_2026-08-08.py の after-fix 版。 baseline (この
ファイルの原本) との差分は以下 2 点のみ:
  1. RecognitionPipeline.load_default に
     enable_ojama_entry_gravity_settle_guard=True (修正B) と
     enable_gravity_settle_reset_on_exit=True (修正C) を追加。
  2. 出力ファイル名に _afterfix サフィックスを付与し baseline の証跡を
     上書きしない。
...
T_START = 45.0
T_END = 75.0
OUT_JSONL = _ROOT / "data/verify/youtube_demo_2026-08-07/_diag_2p_osc_transitions_afterfix_2026-08-08.jsonl"
OUT_DETECT_JSONL = _ROOT / "data/verify/youtube_demo_2026-08-07/_diag_2p_osc_detect_events_afterfix_2026-08-08.jsonl"
...
def main() -> int:
    ...
    pipeline = RecognitionPipeline.load_default(
        ...
        # 修正B+C (2026-08-08、振動バグ B+C の修正) を ON にした効果測定。
