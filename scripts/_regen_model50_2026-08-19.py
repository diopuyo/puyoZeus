...
def _cookie_args(target_id: str) -> list:
    if not (_COOKIES_MASTER.exists() and _COOKIES_MASTER.stat().st_size > 0):
        return []
    work = _COOKIES_MASTER.parent / f"yt_cookies_work_m50_{target_id}.txt"
    try:
...
if __name__ == "__main__":
    base = PROJECT_ROOT / "data" / "verify" / "regen_2026-08-19_model50"
    orch.MANIFEST_TSV = base / "manifest.tsv"
    orch.STATUS_TSV = base / "status.tsv"
    orch.NEW_NPZ_DIR = PROJECT_ROOT / "data" / "indicators_v2" / "boards_lean_model50_2026-08-19"
    orch.LOG_DIR = PROJECT_ROOT / "logs" / "regen_model50_2026-08-19_per_video"
