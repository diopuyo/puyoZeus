"""c21系持続誤認7件「消去後の色残留」の機構確定 (2026-08-17)。

本体コード変更なし (計装は全て外部から instance 属性を読むのみ、
src/ への一時挿入も行わない)。

対象: data/verify/diag_w23_continuous_2026-08-17/c21_chunk1.json, c21_chunk2.json
の7セル (4フレーム起点: 051_c21_1P_f81796, 057_c21_1P_f81890,
004_c21_1P_f144486, 058_c21_2P_f143682)。

各セルについて、統一測定 構成F と同一 pipeline 構成 (force_in_match=False、
連続処理) を対象窓周辺のみ (LOOKBACK_SEC、既知の match_started 実測値から
安全マージンを引いて短縮) 実行し、以下を記録する:
  - フレーム毎の cnn_board 観測値・confirmed_board 確定値
  - cycle71n override の投票履歴 (_stable_cnn_history_{1p,2p]) のスナップショット
    (最新 STABLE_CNN_HISTORY_FRAMES=18 個の cnn 観測値、最頻値比率)
  - override_color_guard の watch リスト登録有無・期限
  - landing grace / chain_exit_warmup の活性状態
  - 独立 HSV-only 分類 (cnn とは別に、フレームから直接 classify) との一致
これにより「誰が書いたか」「なぜ直らないか」を計装ログの数値で確定する。

使い方:
    PYTHONPATH=. ./venv/bin/python -m scripts._diag_c21_burst_2026-08-17 --run c21_chunk1
    PYTHONPATH=. ./venv/bin/python -m scripts._diag_c21_burst_2026-08-17 --run c21_chunk2
"""
...
OUT_DIR = _ROOT / "data" / "verify" / "diag_c21_burst_2026-08-17"
FRAMES_DIR = OUT_DIR / "frames"

STABLE_CNN_HISTORY_FRAMES = 18  # src/recognition_pipeline.py と同値 (計装側では独立に定義、参照専用)
STABLE_OVERRIDE_MIN_RATIO = 0.75
...
# start_sec/end_sec: pipeline を実際に continuous 実行する範囲。
# c21_chunk1.json / c21_chunk2.json の match_started_hist 実測値から、
# 対象窓を含む直近の試合開始点より手前に安全マージン(約45秒)を取って短縮
# (300秒フルlookbackは不要と判断、根拠: 直近試合開始 1355.07s(chunk1)/
# 2386.7s(chunk2)、いずれも対象窓の数秒~十数秒前で history はその時点で
# clear されるため、それより前の長い履歴は本機構に無関係)。
RUNS: dict[str, RunSpec] = {
    "c21_chunk1": RunSpec(
        video_id="c21", start_sec=1320.0, end_sec=1385.0,
        log_lo=1348.0, log_hi=1386.0,
        cells=(
            TargetCell("051_c21_1P_f81796", "1P", 9, 5, 4, 0),
            TargetCell("051_c21_1P_f81796", "1P", 10, 5, 4, 0),
            TargetCell("057_c21_1P_f81890", "1P", 8, 3, 4, 0),
            TargetCell("057_c21_1P_f81890", "1P", 9, 3, 5, 0),
        ),
        note="直近試合開始 t=1355.07 (実測)。start_sec はその35秒前。",
    ),
    "c21_chunk2": RunSpec(
        video_id="c21", start_sec=2340.0, end_sec=2420.0,
        log_lo=2379.0, log_hi=2420.0,
        cells=(
            TargetCell("004_c21_1P_f144486", "1P", 4, 0, 9, 3),
            TargetCell("058_c21_2P_f143682", "2P", 5, 1, 1, 0),
            TargetCell("058_c21_2P_f143682", "2P", 6, 1, 2, 0),
        ),
        note="直近試合開始 t=2386.7 (実測)。start_sec はその46秒前。",
    ),
}
...
def _hsv_mean_at(frame_bgr: np.ndarray, side: str, r: int, c: int) -> list[float] | None:
    """該当セルパッチの平均 HSV (H,S,V) を返す (半透明バースト有無の実測材料)。"""
    region = DEFAULT_P1_REGION if side == "1P" else DEFAULT_P2_REGION
    patch = _extract_cell_patch_from_frame(frame_bgr, region, r, c)
    if patch is None or patch.size == 0:
        return None
...
def run_spec(name: str, save_frames: bool) -> dict:
    ...
    per_cell_log: dict[str, list[dict]] = {
        f"{t.side}_r{t.r}c{t.c}": [] for t in spec.cells
    }
    prev_conf: dict[str, int | None] = {k: None for k in per_cell_log}
    transitions: dict[str, list[dict]] = {k: [] for k in per_cell_log}
    ...
    while t_sec < spec.end_sec:
        ...
        in_log_window = spec.log_lo <= t_sec <= spec.log_hi
        for t in spec.cells:
            ...
            if changed:
                ...
                if save_frames and frame_idx not in saved_frame_ranges:
                    _save_evidence_frames(cap, video_path, fps, frame_idx, t, name)
                    saved_frame_ranges.add(frame_idx)
                prev_conf[key] = conf_val

            if in_log_window:
                per_cell_log[key].append({
                    "frame_idx": frame_idx, "t_sec": round(t_sec, 4),
        ...
        "cells": [
            {
                ...
                "series": per_cell_log[f"{t.side}_r{t.r}c{t.c}"],
            }
            for t in spec.cells
        ],
    }
...
def _save_evidence_frames(
    cap: cv2.VideoCapture, video_path: Path, fps: float, center_frame_idx: int,
    ...
    for delta in range(-3, 4):
        ...
        # 盤面全体も分かるように region 全体にも薄枠
        cv2.rectangle(
            vis, (region.x, region.y),
            (region.x + region.width, region.y + region.height), (0, 255, 255), 1,
        )
        fname = (
            f"{run_name}_{target.sheet_id}_{target.side}_r{target.r}c{target.c}_"
            f"f{f_idx}_d{delta:+d}.png"
        )
