...
def _capture_frames(video_stem: str, start_sec: float, max_sec: float) -> dict[str, list[_FrameRecord]]:
...
def _review_one_video(video_stem: str, start_sec: float, max_sec: float, sim: ChainSimulator) -> dict:
...
def main() -> None:
    ...
    for stem, start_sec, max_sec in TARGET_WINDOWS:
        video_reports.append(_review_one_video(stem, start_sec, max_sec, sim))
