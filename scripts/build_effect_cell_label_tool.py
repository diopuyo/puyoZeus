"""エフェクト有無 セルラベル付け用 クリック操作HTMLツール 生成 (2026-08-03)。

scripts/build_effect_cell_label_sheet.py が準備した labeling_sheet.csv +
frames/*_board_crop.png (~40候補) から、user が盤面クロップ画像の上に直接重ねた
透明グリッドをクリックするだけで「エフェクトが被っているセル」をラベル付けできる
ローカルHTMLツールを生成する。

## 既存クリック式ツール資産の流用
- 座標系 (可視12行×6列、CELL_W/CELL_H) は scripts/visualize_recognition.py /
  scripts/build_full_board_label_tool.py と完全に一致させる (盤面クロップ画像は
  build_effect_cell_label_sheet.py が同じ crop_visible_board_region で
  生成済みのため、座標系はそのまま流用できる)。
- ヘッダー(進捗+ダウンロードボタン)・localStorage自動保存・status badge等の
  UI骨格は build_full_board_label_tool.py の設計を踏襲する (user承認済みUX)。

## 差分 (色ぷよラベルとの違い)
- 色サイクル(8状態)ではなく、エフェクト有無の3状態 (0=なし/1=バースト/2=煙) を
  クリックで循環させる。マーク済みセルは半透明の色オーバーレイで画像の上に
  重ねて表示する (下の実画面が透けて見える必要があるため)。
- 各候補は単一ペイン (盤面クロップ画像+透明グリッドオーバーレイ) のみで、
  色グリッド比較パネルは不要 (エフェクト判定は実画面を直接見て行うため)。

## 出力
    data/verify/effect_cell_label_2026-08-03/label_tool.html
...
Usage:
    PYTHONPATH=. ./venv/bin/python -m scripts.build_effect_cell_label_tool
"""
...
from scripts.build_effect_cell_label_sheet import (  # noqa: E402
...
STORAGE_KEY_PREFIX: str = "puyo_effect_cell_label_tool::"
...
RESULT_CSV_HEADER: tuple[str, ...] = (
    "video_id", "t_sec", "side", "layer", "chain_bin", "status",
...
def build_tool_candidate(row: dict, out_dir: Path) -> "EffectToolCandidate | None":
    ...
    return EffectToolCandidate(
        ...
        layer=row["layer"], chain_bin=row.get("chain_bin", ""),
...
def _render_one_candidate_html(index: int, c: EffectToolCandidate) -> str:
    ...
    chain_note = f" / 連鎖規模:{c.chain_bin}" if c.chain_bin else ""
    title = f"#{index + 1} {c.video_id} {c.side} t={c.t_sec}秒 ({layer_ja}{chain_note})"
    return f'''
...
def _parse_args() -> argparse.Namespace:
    """コマンドライン引数を定義・解析する。"""
    parser = argparse.ArgumentParser(description="エフェクト有無セルラベル付けHTMLツール生成")
...
<head>
<meta charset="utf-8">
<title>エフェクト有無 セルラベル付けツール</title>
<style>{css}</style>
</head>
<body>
<header>
...
function setStatus(index, status) {{
  ...
  if (status === STATUS_NO_EFFECT) STATE[cand.key].grid = emptyGrid();
...
function buildResultCsv() {{
  ...
  for (const c of CANDIDATES) {{
    ...
    const effectGrid = status === STATUS_SKIP ? "" : encodeGridString(st.grid);
    lines.push([c.video_id, c.t_sec, c.side, c.layer, c.chain_bin, status,
