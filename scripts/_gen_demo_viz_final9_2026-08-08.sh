#!/bin/bash
# FINAL9: FINAL8 から連鎖数表示を外した版 (連鎖中ずっと chain 表示のみ)
# あってほしい」、 2026-08-08)。
...
CMD_A="nice -n 19 ./venv/bin/python -u -m scripts.visualize_recognition --video $IN --output data/verify/youtube_demo_2026-08-07/dio_vs_ts_FINAL9a_all_states_viz.mp4 $BASEFLAGS"
CMD_B="nice -n 19 ./venv/bin/python -u -m scripts.visualize_recognition --video $IN --output data/verify/youtube_demo_2026-08-07/dio_vs_ts_FINAL9b_stable_tsumo_viz.mp4 $BASEFLAGS --overlay-show-states stable,tsumo_fall --overlay-state-debounce-frames 10"

{ echo "[cmd] $CMD_A"; eval "$CMD_A"; } > logs/demo_viz_final9a_2026-08-08.log 2>&1 &
{ echo "[cmd] $CMD_B"; eval "$CMD_B"; } > logs/demo_viz_final9b_2026-08-08.log 2>&1 &
wait
echo "FINAL9_BOTH_DONE $(date)"
ls -lh data/verify/youtube_demo_2026-08-07/dio_vs_ts_FINAL9*.mp4
