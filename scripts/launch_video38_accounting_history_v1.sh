...
output_root="${1:?新規会計履歴診断出力directoryを指定してください}"
...
setsid -f env PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. CUDA_VISIBLE_DEVICES=0 \
  ...
  -m scripts.diagnose_video38_accounting_history_v1 \
  --allow-native-runtime-mismatch --output-root "$output_root" \
  > "$log_path" 2>&1 < /dev/null
sleep 1
history_pid="$(pgrep -n -f "^${project_root}/venv/bin/python -u -m scripts.diagnose_video38_accounting_history_v1 .*--output-root ${output_root}$")"
printf 'PID=%s\nLOG=%s\nOUTPUT=%s\n' "$history_pid" "$log_path" "$output_root"
