#!/bin/bash
# #43 段階2: マスター級先行20本の指標収集ジョブ生成。
# video_c{n} を $HOME/frames (ext4) へコピーし、base/gap/mid の3窓ジョブを
# _jobs_labeled_win_m20_2026-07-28.txt に書き出す (scripts/_run_safe.sh で消化する形式)。
# 窓定義は 2026-07-26 の c20 収集と同一 (踏襲):
#   base = 0-300s / gap = 300-900s (--start-sec 300 --max-sec 600) /
#   mid  = 1200-1560s (--start-sec 1200 --max-sec 360)
#
# 使い方: bash scripts/_gen_jobs_labeled_win_m20_2026-07-28.sh <selected_videos_m20.txt>
set -u
PROJ_DIR="/mnt/c/Users/ryouj/.gemini/antigravity/scratch/puyo_analyzer"; cd "${PROJ_DIR}" || exit 1

VIDEO_LIST="${1:?selected_videos_m20.txt (1行1 video_id) が必要}"
OUT_STUDY_DIR="data/verify/labeled_win_m20_2026-07-28/study"
...
while IFS= read -r vid; do
  ...
  for suf_args in "|--max-sec 300" "_gap|--start-sec 300 --max-sec 600" "_mid|--start-sec 1200 --max-sec 360"; do
    suf="${suf_args%%|*}"
    args="${suf_args#*|}"
    echo "PYTHONPATH=. ./venv/bin/python -m scripts._collect_1t --video \$HOME/frames/video_c${n}.mp4 --out ${OUT_STUDY_DIR}/c${n}${suf}.csv ${args} > logs/labeled_win_m20_c${n}${suf}_2026-07-28.log 2>&1"
  done >> "${JOBS}"
done < "${VIDEO_LIST}"
