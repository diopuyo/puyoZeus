...
MASTER="${LOGDIR}/master_glow_v4.log"; echo "[start] glow v4 A/B $(date)" > "${MASTER}"
PYTHONPATH=. venv/bin/python scripts/measure_stable_cell_acc.py \
  ...
  --output "${OUTDIR}/corruption_glow_v4_2026-06-04.json" > "${LOGDIR}/eval_glow_v4.log" 2>&1
echo "[eval] glow v4 done $(date)" >> "${MASTER}"
PYTHONPATH=. venv/bin/python scripts/visualize_recognition.py \
  --video "${VDIR}/v89/v89_match01.mp4" --output "${VIZDIR}/v89_match01_glowV4_2026-06-04.mp4" --ojama-warning-glow-guard \
  --dump-board-log-detailed "${VIZDIR}/v89_match01_glowV4_2026-06-04.jsonl" > "${LOGDIR}/viz_v89_glowV4.log" 2>&1
echo "[done] glow v4 全完了 $(date)" >> "${MASTER}"
