"""W15-C: CNN v16 - v15 init + review oversample 100倍 + class 制限緩和。

v15 (review fine-tune 12 epoch) を init として更に 15 epoch 学習。
review labels を 100倍 oversample (= ~150K + 4750 unique cells から hard cells を強学習)。
class 制限を 20K に緩めて review の hard cells が希釈されにくく。
"""
...
def main() -> int:
    ...
    parser.add_argument(
        "--init-model", default="models/cnn_phase_u_v15.pt",
    )
    parser.add_argument(
        "--out-model", default="models/cnn_phase_u_v16.pt",
    )
    parser.add_argument(
        "--out-dataset",
        default="data/training_phase_u/v16_dataset.npz",
    )
    parser.add_argument(
        "--review-multiplier", type=int, default=100,
    )
