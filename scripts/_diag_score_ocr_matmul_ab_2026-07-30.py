"""スコアOCR行列積NCC (enable_score_ocr_matmul) の採否検証 (2026-07-30)。

行列積経路は cv2(float32) と numpy(float64) の差でスコアに最大 5.5e-07 の乖離が
出るため **bit-identical ではない**。閾値 (_min_confidence 0.55 / _margin_min 0.04)
の境界ぴったりでは判定が変わりうるので、実動画で以下を測る:
...
def _run(frames: list[np.ndarray], matmul: bool) -> tuple[list[tuple], float]:
    """パイプラインを 1 本走らせ、(フレーム毎スナップショット, 定常中央 ms)。"""
    from src.recognition_pipeline import RecognitionPipeline

    pipe = RecognitionPipeline.load_default(enable_score_ocr_matmul=matmul)
    snaps: list[tuple] = []
    times: list[float] = []
...
def main() -> None:
    ...
    for name in args.videos:
        ...
        if not frames:
            ...
            continue
        off, ms_off = _run(frames, matmul=False)
        on, ms_on = _run(frames, matmul=True)
    ...
    print(
        f"OCRスコア差分 {tot_score_diff}/{tot_frames} フレーム "
    ...
    if tot_score_diff == 0 and tot_board_diff == 0:
        print("→ 差分ゼロ。float64 化の乖離は判定に影響していない。")
    else:
        print(
