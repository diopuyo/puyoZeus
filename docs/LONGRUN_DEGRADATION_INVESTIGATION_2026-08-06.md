...
## 追補: 第4機構の確定と修正A' (2026-08-06 14:45)
- **真因確定 (c22_reset_trace計装)**: ①②③ (較正系) は機能していた。真因=第4機構:
  `_match_active_started_time` が is_active の遷移で無条件に再アーム/リセットされ、
  **試合中0.34秒のMENU誤検知**で15秒ガードが二重再起動→33秒の無防備窓が
  「自連鎖直後の列消去 (94%再構築からの逆転)」の修復機会を完全に塞いだ。
  ガード失効後は resync 正常作動 (10/12回復)
- 「長時間劣化」の正体 = 時間蓄積ではなく「境界誤検知のたびに開く無防備窓」
  (試合数∝機会増 → game_idx弱相関とも整合)
- **修正A' (アーキ裁定)**: 書き込み地点2箇所 (True/False遷移) を対称デバウンス
  (「直前状態がN秒以上継続してから確定」)。片側のみのAは bg_fp 瞬間ロス等を
  放置する局所パッチとして撤回。N = max(フリッカー長) < N < min(真のMENU期間)
  を boundary_events 実測で両側確認してから固定 (シーン逆算禁止)
- cycle_25再燃チェック必須: 短い試合間遷移の動画でガードの15秒再武装をログ確認
- B (MatchEndDetector誤検知の根治) は系統性未証明のため保留 (全89動画での
  「試合中0.5秒未満MENU誤検知」頻度計測が先)
