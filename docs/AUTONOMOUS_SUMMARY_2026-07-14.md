...
- **多ゲーム勝者一致率(2動画/10ゲーム = 9/10 = 90%)**:
  - v29 [140-420] 4ゲーム → **3/4=75%**(外し1件はスコア極小415/1309の短い低得点ゲーム、
    有利不利ほぼ互角+9=低シグナルで本質的に難)。
  - v30 [150-450] 6ゲーム(別動画 light vs あん) → **6/6=100%**(近接ゲーム-7/+8/-2でも符号一致)。
  - 明確なゲームは確実に、近接ゲームも概ね勝者を正しく捉える。
- **性能**: 認識(pipe.update毎フレーム)+満杯盤面のsimが律速。HeavyAdvCacheで重い
  盤面由来計算を~0.3s毎に間引き改善したが、認識自体は残り~6x realtime(1試合数分)。
  認識高速化は別領域(Phase I)。
