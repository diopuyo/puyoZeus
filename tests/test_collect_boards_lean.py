...
def test_collect_lean_signature_has_sample_interval_frames_appended_at_tail() -> None:
    ...
    assert params[-10] == "enable_ojama_fall_placement_override"
    assert (
        sig.parameters["enable_ojama_fall_placement_override"].default is False
    )
    assert params[-9] == "enable_ojama_fall_entry_hardening"
    assert sig.parameters["enable_ojama_fall_entry_hardening"].default is False
    assert params[-8] == "enable_chain_gate_raw_fallback"
    ...
    assert params[-7] == "enable_ojama_fall_scoped_exit"
    ...
    assert params[-5] == "enable_highlight_override"
    ...
    assert params[-4] == "enable_patch_fp_hsv_guard"
    ...
    assert params[-1] == "enable_floating_gap_restore"
